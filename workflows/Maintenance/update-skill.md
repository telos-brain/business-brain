...
back. That is not a reason to capture more. If the source does not clear the
quality bar below, make no schema changes. Zero skills is a successful run.
...
return without changes. If it is only a client, a project, a person, or an
implementation with no transferable practice, stop without changes.

#### Quality bar: what earns extraction

Extract only if it is **transferable and reusable** — a practice, standard,
process, or expertise an expert at this company would deliberately teach the
next person:

- **Discard noise.** Greetings, small talk, tangents, weekly plans, hedging,
  abandoned ideas, personal life.
- **Discard generic truisms.** Obvious advice that adds nothing specific.
- **Strip the case.** Remove client names, project names, people, ticket ids,
  and one-off configuration. If nothing teachable remains, it is not a skill.
- **Never include implementation details** that only applied once — a
  customer's settings, a project's file layout, a price agreed for one deal.
- **Never include PII or customer specifics** in skill content.

When a candidate fails this bar, leave it out. Do not go hunting because a
category exists. One strong skill — or zero — is a success.

#### Which book

- **Company (`CMP`)** is where this organisation's methods go. If that book's
  description still contains `Starter placeholder`, the book has not been
  configured: do **not** create or edit skills in it. Stop that part of the
  run. Onboarding replaces the placeholder first.
- **Advisory, Decision Making, and Business** hold general craft that shipped
  with the brain. Update one only when the source genuinely refines that
  general craft. Do not file this company's operating procedure into those
  books.
- **Telos Brain (`BRA`)** is the platform. Do not add company practices to it.
...
3. **Contamination check:** Does any new or edited skill still contain a
   client, project, person, or one-off implementation detail? Remove it, or
   drop the skill if that was all it was.
4. **Precision check:** Remove or merge anything trivial, generic, redundant or
   derived from noise. Do not add a skill at this stage to "cover" a topic
   you correctly skipped.
...
- **In-scope reusable knowledge:** when in doubt whether it is really a
  repeatable practice, leave it out.
