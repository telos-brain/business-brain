...
| One client | Client blueprint, one file per client | How the company does this kind of work in general |
| One project or job | Job blueprint, one file per job | The standing client file, or the company's method |
...
Defaults already in the schema: clients (`clients`) and jobs (`jobs`). Accept the default when they are unsure and they clearly serve clients.

- No client roster (a product sold to a market): remove the `clients` entity from `brain-compose.yml` and remove `blueprints/clients/blueprint.yml` from the compose list. Company-level **Customers** is enough.
- Clients, but no projects: remove the `jobs` unit of work and `blueprints/jobs/blueprint.yml`.
- They use another word (engagements, matters, accounts): rename the `code` to a lowercase hyphen-free word and point the blueprint `scope.code` at it. Confirm the code with them only if the word is ambiguous.
...
| Customers | Who the business serves, as a portfolio. Not one client's file. |
| Products and services | One thing the business sells. Not the delivery process. |

**Client** (`blueprints/clients/blueprint.yml`) — isolated per client:

| Category | One entry is |
|---|---|
| People | One person at that client, and their role in the work. |
| Commercial | The deal with that client: what is sold, terms already agreed. |
| Situation | What is in play that will still matter next month. Not this week's list. |

**Job** (`blueprints/jobs/blueprint.yml`) — isolated per job:

| Category | One entry is |
|---|---|
| Brief | What this work is and what done looks like. One entry, updated. |
| Decisions | One decision that should not be relitigated. |
| State | Where it stands: owner, status, what is blocking. One entry, updated. |
...
1. Edit `brain-compose.yml` (name stays **Business**), the Company skill book, and the blueprints that still apply. Delete compose lines for blueprints you removed.
...
**Sol** is the advisor's persona — how Ask Sol speaks. The MCP server is **Business** (`WF-BUSINESS`). The brain's title in `brain-compose.yml` is **Business**.
...
4. Deploy:
   ...
   ```bash
   brain deploy --instance business
   ```
...
2. Open this brain (instance **business**, title **Business**).
...
- A meeting not tied to a client updates the **company** record only, and only when a fact fits Team, Strategy, Systems, Customers, or Products and services.
- A specific client's people, deal, or live work is written on that client when the run is in that client's context. It is not copied onto the company record to avoid losing it. If the intake is not attached to a client, those facts are left out.
- A job's own file is for work that runs as that job. An unscoped transcript does not fill it.
...
```bash
brain deploy --env local --instance business
```
